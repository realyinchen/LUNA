...
def enumerateWithEstimate(iter_obj, desc_str, start_ndx=0, print_ndx=4, backoff=None, iter_len=None):
    """
    ...
    :param iter_obj: `iter_obj` is the iterable that will be passed into
        `enumerate`. Required.
    ...
    for (current_ndx, item) in enumerate(iter_obj):
        ...
        if current_ndx == print_ndx:
            ...
            log.info("{} {:-4}/{}, done at {}, {}".format(
                desc_str, current_ndx, iter_len, str(done_dt).rsplit('.', 1)[0], str(done_td).rsplit('.', 1)[0]))
    ...
    log.warning("{} ----/{}, done at {}".format(desc_str, iter_len, str(datetime.datetime.now()).rsplit('.', 1)[0]))
