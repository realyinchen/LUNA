...
from dataset.dataset import Luna2dSegmentationDataset, TrainingLuna2dSegmentationDataset, getCt
from models.segmentation_model import UNetWrapper, SegmentationAugmentation
...
class SegmentationTrainingApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--augment-scale',
                            help="Augment the training data by randomly increasing or decreasing "
        ...
        parser.add_argument('comment', help="Comment suffix for Tensorboard run.", nargs='?', default='none')
    ...
    def initTrainDl(self):
        train_ds = TrainingLuna2dSegmentationDataset(val_stride=10, isValSet_bool=False, contextSlices_count=3)
        ...
        train_dl = DataLoader(
            train_ds, batch_size=batch_size, num_workers=self.cli_args.num_workers, pin_memory=self.use_cuda)
    ...
    def initValDl(self):
        ...
        val_dl = DataLoader(
            val_ds, batch_size=batch_size, num_workers=self.cli_args.num_workers, pin_memory=self.use_cuda)
    ...
    def doValidation(self, epoch_ndx, val_dl):
        with torch.no_grad():
            ...
            batch_iter = enumerateWithEstimate(
                val_dl, "E{} Validation ".format(epoch_ndx), start_ndx=val_dl.num_workers)
            for batch_ndx, batch_tup in batch_iter:
    ...
    def logImages(self, epoch_ndx, mode_str, dl):
        ...
        for series_ndx, series_uid in enumerate(images):
            ...
            for slice_ndx in range(6):
                ...
                writer.add_image(f'{mode_str}/{series_ndx}_prediction_{slice_ndx}',
                                 image_a, self.totalTrainingSamples_count, dataformats='HWC')
                ...
                if epoch_ndx == 1:
                    ...
                    image_a[image_a < 0] = 0
                    image_a[image_a > 1] = 1
                    writer.add_image('{}/{}_label_{}'.format(mode_str, series_ndx, slice_ndx),
                                     image_a, self.totalTrainingSamples_count, dataformats='HWC')
    ...
    def saveModel(self, type_str, epoch_ndx, isBest=False):
        file_path = os.path.join('data-unversioned', 'part2', 'models', self.cli_args.tb_prefix,
                                 '{}_{}_{}.{}.state'.format(
                                     type_str, self.time_str, self.cli_args.comment, self.totalTrainingSamples_count))
        ...
        if isBest:
            best_path = os.path.join('data-unversioned', 'part2', 'models', self.cli_args.tb_prefix,
